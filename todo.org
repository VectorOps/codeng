...
** [ ] Rethink tool step handling. Make it easier for UI to understand the tool execution state
- Create a new tool step type instead of PROMPT
- Add a status to tool req (pending_approval, running, complete), emit statuses to update tool req in UI (spinner, etc)
** [X] Create Step renderer in TUI client
** [X] Add input request stack to TUI
** [ ] Set input header depending on the top of the input stack
* [ ] Runner
