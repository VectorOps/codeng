...
class Runner:
    ...
    async def run(self) -> AsyncIterator[RunEvent]:
        ...
        while True:
            if current_execution is None or current_execution.node != current_runtime_node.name:
                current_execution = self._ensure_node_execution(current_runtime_node.name)

            executor = self._executors[current_runtime_node.name]
            executor_input = ExecutorInput(execution=current_execution, run=self.execution)
