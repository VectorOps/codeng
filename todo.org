...
* [ ] General
** [X] Support streaming MCP transports
*** [X] Introduce intermediate tool response support
*** [X] Figure out how to handle bi-directional communication with MCP
*** [X] Update runner, tool call manager, etc
** [X] Parallel tool calling
** [X] Analyze why usage for current session is not being captured correctly with Connect, as if input messages from previous round are not sent over
** [X] Come up with a plugin architecture for LLM authentication logic (OAuth2, etc)
** [X] knowlt: reject virtual path writes
** [ ] Nicely format tool call rejection text
** [X] Implement combined input/http_input component
*** [X] Think about unified input executor
** [X] If tool call is just approved, do not generate component with "User approved." text.
** [ ] Update task list error needs to be formatted (or not displayed)
** [X] Add dynamic progress indicator for knowlt
** [X] Investigate lag when sending first input message
** [X] Remove extra line break from task tool
** [ ] Add a way to hide subagent output and make it non-interactive
** [X] Add dynamic variable support that's calculated at runtime
*** [X] Add a way to change variable setting via TUI
- List variables
...
** [X] Fix up logging
*** [X] LiteLLM critical errors should be intercepted and displayed via existing rendering machinery
** [ ] Editing history with subagent does not delete subagent components
** [X] Tool call rejection with the message needs format
** [X] Add commands to tool approval that change behavior (auto-approve tool for this session, etc)
** [ ] Know: rename read_files to read_file
** [X] Auto-complete
*** [X] Space with selection should auto-complete
** [X] Send LLM stats when executor starts (from old state?)
** [X] Store cached stats in LLM usage stats
** [X] Create alternative Markdown renderer that only highlights, but does not format
** [X] Need apply_patch formatter with input and output (collapsed)
** [-] Assign a number to collapse/expand a specific component and show number in the component view. (bad idea due to repaints?)
** [X] Compact tool call representation, where consequent calls are merged onto a single.
** [X] Hide time indicator for tool calls - it is not helpful
** [X] Limit number of lines actually rendered for full refreshes, make the number configurable
** [X] Combine input + toolbar + help + etc to a single component due to dynamic component shifting
...
** [X] Add /reset command
** [X] Test multi-screen paste
*** [X] Handle paste without showing whole content
...
** [ ] Simplify UI history protocol semantics
- Replace compatibility-style step deletion semantics with explicit visible-history diff and capability-negotiated branch-aware packets
** [X] Gemini 3 support
*** [X] Temperature control
** [X] Add ctrl+r history search
** [ ] Add context compression? How?
** [X] Collapse/expand to work with visible components first, then continue going backwards. Keep track of updated components to resume from.
* [ ] Settings
...
** [ ] Text parser (any file that is ASCII)
* [ ] Worktree support
