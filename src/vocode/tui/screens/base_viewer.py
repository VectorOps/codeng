...
class BaseViewerScreen(abc.ABC):
    ...
    def _build_body_renderable(self) -> rich_console.RenderableType:
        height = self._height()
        ...
        for offset, line in enumerate(raw_lines):
            ...
            text = rich_text.Text(line, no_wrap=True, overflow="crop")
            if index == self._cursor_line and self._search_pattern is not None:
        ...
        return rich_text.Text("\n").join(body_lines).truncate(width)
    ...
    def _build_footer_renderable(self) -> rich_console.RenderableType:
        ...
        help_text = "q: quit  j/k, up/down: line  f/b, pgdn/pgup, space: page  /: search  n: next"
        separator = (
    ...
    def render(self) -> None:
        ...
        if height <= 0:
            return
        self._clamp_state()
        body = self._build_body_renderable()
